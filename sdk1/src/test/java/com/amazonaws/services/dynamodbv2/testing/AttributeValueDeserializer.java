...
public class AttributeValueDeserializer extends JsonDeserializer<AttributeValue> {
    @Override
    public AttributeValue deserialize(JsonParser jp, DeserializationContext ctxt) throws IOException, JsonProcessingException {
        ...
        for (Iterator<Map.Entry<String, JsonNode>> iter = attribute.fields(); iter.hasNext(); ) {
            ...
            switch (typeString) {
                ...
                default:
                    throw new IllegalStateException("DDB JSON type " + typeString + " not implemented for test attribute value deserialization.");
            }
        }
        return null;
    }
