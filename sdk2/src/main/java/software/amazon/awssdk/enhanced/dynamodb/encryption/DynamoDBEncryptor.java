...
import software.amazon.awssdk.enhanced.dynamodb.encryption.utils.EncryptionContextOperators;
import software.amazon.awssdk.enhanced.dynamodb.internal.AttributeValueMarshaller;
import software.amazon.awssdk.enhanced.dynamodb.internal.ByteBufferInputStream;
import software.amazon.awssdk.enhanced.dynamodb.internal.Utils;
import software.amazon.awssdk.services.dynamodb.model.AttributeValue;

/**
 * The low-level API used by {@link EncryptedRecordExtension} to perform crypto operations on the record
 * attributes.
 *
...
public class DynamoDBEncryptor {
  ...
  public Map<String, AttributeValue> encryptRecord(
    ...
    // Copy to avoid changing anyone else's objects
    itemAttributes = new HashMap<String, AttributeValue>(itemAttributes);
    ...
    AttributeValue signatureAttribute =
            AttributeValue.builder()
                    .b(SdkBytes.fromByteArray(signature))
                    .build();
  ...
  protected static AttributeValue marshallDescription(Map<String, String> description) {
    try {
      ...
      return AttributeValue.builder()
              .b(SdkBytes.fromByteArray(bos.toByteArray()))
              .build();
  ...
  protected static Map<String, String> unmarshallDescription(AttributeValue attributeValue) {
    ByteBuffer buffer = attributeValue.b().asByteBuffer();
    buffer.mark();
    try (DataInputStream in =
        new DataInputStream(new ByteBufferInputStream(buffer))) {
