...
import software.amazon.awssdk.enhanced.dynamodb.encryption.materials.AsymmetricRawMaterials;
...
public class KeyStoreMaterialsProvider implements EncryptionMaterialsProvider {
  ...
  private final AtomicReference<CurrentMaterials> currMaterials =
      new AtomicReference<CurrentMaterials>();
    ...
    if (context.getMaterialDescription().entrySet().containsAll(description.entrySet())) {
      if (materials.encryptionEntry instanceof SecretKeyEntry) {
        ...
        try {
          ...
          throw new EncryptionException("Unable to decrypt envelope key", ex);
        }
      }
    } else {
      return null;
  ...
  public EncryptionMaterials getEncryptionMaterials(EncryptionContext context) {
    ...
    if (materials.encryptionEntry instanceof SecretKeyEntry) {
      ...
      try {
        ...
        throw new EncryptionException("Unable to encrypt envelope key", ex);
      }
    }
  }
  ...
  public void refresh() {
    try {
      ...
      throw new EncryptionException("Unable to load keys from keystore", ex);
    }
  }
