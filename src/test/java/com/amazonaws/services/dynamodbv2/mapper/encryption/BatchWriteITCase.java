...
public class BatchWriteITCase extends DynamoDBMapperCryptoIntegrationTestBase {
    ...
    public void testBatchSaveAsArray() throws Exception {
        ...
        DynamoDBMapper mapper = TestDynamoDBMapperFactory.createDynamoDBMapper(dynamo);
        NumberSetAttributeTestClass[] objsArray = objs.toArray(new NumberSetAttributeTestClass[0]);
    ...
    public void testLargeRequestEntity() {
        ...
        // no longer 3 due to the increase of coral http request size from 1MB to 16MB
//      assertTrue(""+size, 3 == size);
        ...
        if (DEBUG)
            System.err.println("failedBatches.size()=" + size);
        // no longer 5 due to the increase of coral http request size from 1MB to 16MB
//        assertTrue(""+ size, 5 == failedBatches.size());
        assertEquals("" + size, 1, failedBatches.size());
    }
