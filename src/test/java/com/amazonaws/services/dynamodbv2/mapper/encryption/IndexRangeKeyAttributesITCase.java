...
public class IndexRangeKeyAttributesITCase extends DynamoDBMapperCryptoIntegrationTestBase {
    ...
    public void testLoad() throws Exception {
        for (Map<String, AttributeValue> attr : attrs) {
            IndexRangeKeyTestClass x = mapper.load(newIndexRangeKey(
    ...
    private IndexRangeKeyTestClass newIndexRangeKey(long hashKey, double rangeKey) {
    ...
    public void testSave() throws Exception {
        ...
        for (IndexRangeKeyTestClass obj : objs) {
            IndexRangeKeyTestClass loaded = mapper.load(IndexRangeKeyTestClass.class, obj.getKey(), obj.getRangeKey());
    ...
    public void testUpdate() throws Exception {
        ...
        for (IndexRangeKeyTestClass obj : objs) {
            ...
            IndexRangeKeyTestClass loadedObject = mapper.load(IndexRangeKeyTestClass.class, obj.getKey(), obj.getRangeKey());
    ...
    public void testQueryWithIndexRangekey() {
        ...
        for (long hashKeyValue : hashKeyValues) {
             ...
             */
            List<IndexRangeKeyTestClass> result = mapper.query(IndexRangeKeyTestClass.class,
            ...
            assertEquals(rangePerHash, result.size());
             ...
             */
            result = mapper.query(IndexRangeKeyTestClass.class,
                    new DynamoDBQueryExpression<IndexRangeKeyTestClass>()
                            .withHashKeyValues(hashKeyItem)
                            .withRangeKeyCondition(INDEX_FOO_RANGE_KEY,
                                    new Condition()
                                            .withAttributeValueList(new AttributeValue().withN("0"))
                                            .withComparisonOperator(ComparisonOperator.GE.toString())));
            assertEquals(indexFooRangePerHash, result.size());
                    ...
                    new DynamoDBQueryExpression<IndexRangeKeyTestClass>()
                            ...
                            .withRangeKeyCondition(INDEX_BAR_RANGE_KEY,
                                    new Condition()
                                            .withAttributeValueList(new AttributeValue().withN("0"))
                                            .withComparisonOperator(ComparisonOperator.GE.toString())));
            assertEquals(indexBarRangePerHash, result.size());
            // check that only the projected attributes are retrieved
    ...
    public void testQueryWithRangeKeyForMultipleIndexes() {
        ...
        for (long hashKeyValue : hashKeyValues) {
            ...
            assertEquals(multipleIndexRangePerHash, result.size());
            // check that only the projected attributes are retrieved
            for (IndexRangeKeyTestClass itemInFooIndex : result) {
                assertNotNull(itemInFooIndex.getFooAttribute());
                assertNotNull(itemInFooIndex.getBarAttribute());
